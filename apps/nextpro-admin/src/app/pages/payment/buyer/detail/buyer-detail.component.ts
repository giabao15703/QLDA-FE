...
import { FormService, LoadingService, NotificationService, PaymentDataService } from '#shared/services';
import { E_FieldType, E_Form_Mode, I_HistoryPayment, I_HistoryPaymentForm } from '#shared/types';
import { getFile } from '#shared/utils';

@Component({
    standalone: true,
    selector: 'nextpro-admin-payment-buyer-detail',
...
export class PaymentBuyerDetailComponent {
    constructor(
    ...
    async ngOnChanges(changes) {
        if (changes?.mode?.currentValue !== E_Form_Mode.CREATE) {
            if (this.data) {
                ...
                this.showUpdateStatus = paymentDetail.methodPayment === 2 && paymentDetail.status === 1;
    ...
    handleUpdateStatus = async (status) => {
        const { historyUpdateStatus } = await this.paymentService.updateHistoryStatus({
            history: {
                id: this.data.id,
                status,
            },
        ...
        if (historyUpdateStatus.status) {
            this.showUpdateStatus = false;
            await this.refetch();
            this.notificationService.success('notification.updateSuccessfully');
            this.onCloseDrawer();
        } else {
            this.notificationService.error(historyUpdateStatus.error?.message);
        }
    };
}
