...
const TAB_NAME = 'bankaccount';

@Component({
    standalone: true,
    selector: 'nextpro-admin-sicp-bank-account-list',
...
export class SicpBankAccountListPage {
    constructor(
    ...
    getUserSupplierSicps = async (variables?: I_QueryVariables) => {
        const userSupplierSicps = await this.sicpService.getUserSupplierSicps(
            {
                ...getQueryVariables({ variables: { ...variables, fileFilter: 1 } }),
            },
            { extra: { variables } },
        );
    ...
    getUserSupplierSicpTextEditor = async (variables?: I_QueryVariables) => {
        const userSupplierSicpTextEditor = await this.sicpService.getUserSupplierSicpTextEditor(
            {
                ...getQueryVariables({ variables: { ...variables, sicpTypeFilter: '1' } }),
            },
            { extra: { variables } },
        );
