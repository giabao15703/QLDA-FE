...
const TAB_NAME = 'duediligence';

@Component({
    standalone: true,
    selector: 'nextpro-admin-sicp-due-diligence-list',
    templateUrl: './due-diligence-list.component.html',
    styleUrl: './due-diligence-list.component.scss',
...
export class SicpDueDiligenceListPage {
    constructor(
    ...
    getUserSupplierSicps = async (variables?: I_QueryVariables) => {
        const userSupplierSicps = await this.sicpService.getUserSupplierSicps(
            {
                ...getQueryVariables({ variables: { ...variables, fileFilter: 3 } }),
            },
            { extra: { variables } },
        );
    ...
    getUserSupplierSicpTextEditor = async (variables?: I_QueryVariables) => {
        const userSupplierSicpTextEditor = await this.sicpService.getUserSupplierSicpTextEditor(
            {
                ...getQueryVariables({ variables: { ...variables, sicpTypeFilter: '3' } }),
            },
            { extra: { variables } },
        );
