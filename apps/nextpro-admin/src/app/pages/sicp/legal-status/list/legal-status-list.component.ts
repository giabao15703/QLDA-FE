...
const TAB_NAME = 'legalstatus';

@Component({
    standalone: true,
    selector: 'nextpro-admin-sicp-legal-status-list',
    templateUrl: './legal-status-list.component.html',
    styleUrl: './legal-status-list.component.scss',
...
export class SicpLegalStatusListPage {
    constructor(
    ...
    getUserSupplierSicps = async (variables?: I_QueryVariables) => {
        const userSupplierSicps = await this.sicpService.getUserSupplierSicps(
            {
                ...getQueryVariables({ variables: { ...variables, fileFilter: 5 } }),
            },
            { extra: { variables } },
        );
    ...
    getUserSupplierSicpTextEditor = async (variables?: I_QueryVariables) => {
        const userSupplierSicpTextEditor = await this.sicpService.getUserSupplierSicpTextEditor(
            {
                ...getQueryVariables({ variables: { ...variables, sicpTypeFilter: '5' } }),
            },
            { extra: { variables } },
        );
