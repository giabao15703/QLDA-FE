...
import { AfterContentInit, DestroyRef, Directive, ElementRef, PLATFORM_ID, inject } from '@angular/core';
...
@Directive({
    standalone: true,
    selector: '[syncTabsWithAnchor]',
})
export class SyncTabsWithAnchorDirective implements AfterContentInit {
    ...
    private listenForTabChanges(): void {
        const tabChangeSub = this.group.selectedTabChange
            .pipe(
                map((event) => {
                    return event.tab.textLabel || 'tab-' + event.index.toString();
                }),
            )
            .subscribe((fragment) =>
